  ...
  module Routes
    class Ordering < Base
      ...
      post '/order/pay-return' do
        ...
        select_order.save()
        ActiveRecord::Base.connection.execute("UPDATE tblOrders SET PaidDate = GetDate(),TransactionDate = (Select top 1 ReportDate from tblStoreReportDate where StoreID = #{select_order[:StoreID]} Order by RADRAT DESC) WHERE OrderID = #{select_order[:OrderID]}")
      ...
      post '/order/complete' do
        ...
        if !params[:payCash].blank?
          ...
          begin
            ...
            ActiveRecord::Base.connection.execute("UPDATE tblOrders SET TransactionDate = (Select top 1 ReportDate from tblStoreReportDate where StoreID = #{select_order[:StoreID]} Order by RADRAT DESC) WHERE OrderID = #{select_order[:OrderID]}")
            ActiveRecord::Base.connection.execute_procedure("WebPrintOrder", {:pStoreID => select_store[:StoreID], :pOrderID => select_order[:OrderID]})
            session[:completeOrder] = select_order[:OrderID]
